...
function renderPitchLesson(l) {
  screen.innerHTML = `
    <div class="lesson-header"><button class="back" data-action="back">←</button><div><strong>${esc(l.title)}</strong><div class="muted">+${l.xp} XP</div></div></div>
    <div class="exercise">
      <div class="eyebrow">${l.type === "breathing" ? "Técnica vocal" : "Afinación"}</div>
      <h1>${l.type === "breathing" ? "Respira y prepara tu voz" : "Canta la nota LA"}</h1>
      <p class="instruction">${l.type === "breathing" ? "Inhala 4 segundos y exhala 6 segundos." : "Pulsa el micrófono y mantén la nota durante 3 segundos."}</p>
      <button class="mic" id="mic">🎤</button>
      <button class="btn btn-primary" id="complete">Comenzar</button>
      <p class="muted" id="micStatus">El acceso al micrófono se incorporará en la siguiente iteración.</p>
    </div>`;
  document.querySelector("#complete").onclick = () => {
    const mic = document.querySelector("#mic"); mic.classList.add("listening");
    document.querySelector("#micStatus").textContent = "Entrenamiento en progreso…";
    setTimeout(() => { mic.classList.remove("listening"); beginSession(l); renderResult(l, l.type === "breathing" ? 96 : 88) }, 2200);
  };
}
